            ...
            <div class="col-md-4 offset-md-4" style="margin-bottom:1em;">
                <form id="search">
                    <div class="input-group">
                        <input type="text" class="form-control form-inline" name="query"
                            placeholder="Search to filter strains" v-model="searchQuery" autofocus>
                        <div v-if="searchQuery !== ''" class="input-group-addon">
                            <button v-on:click="clearSearchQuery()" for="query"
        ...
        <div class="row" v-bind:key="products.uuid" v-for="products in filteredProducts">
            <div class="col-sm-4 info strainlisting" v-bind:key="item.id" v-for="item in products">
                ...
                <a :href="'/strains/' + item.uuid">
                    <h4 style="font-weight:800;">{{item.name}}
                ...
                <a :href="'/strains/' + item.uuid">
                    <img v-if="item.images === null || item.images === undefined" :src="item.image" :alt="item.name"
                        :title="item.name" />
