...
export default {
   ...
   methods: {
    ...
    watch: {
        address: function () {
            if (this.address !== null && this.address !== '' && this.payment_method !== null && this.payment_method !== '') {
                                ...
                                <div class="col-md-12">
                                    <div class="row py-2" style="font-size:1.1em; border-bottom:1px solid #ccc;">
                                        ...
                                        <div class="col-3 d-none d-sm-block">
                                            <img class="info" :src="item.strainImage" :alt="item.strainName" :title="item.strainName" style="width:90px;margin:0.3rem;">
                                        </div>
                                        <div class="col-6">
                                            {{ item.pivot.quantity }}:
                                            {{ item.strainName}} -
                            ...
                            <div class="form-group row">
                                <div class="col-12">
                                    <select id="payment" name="payment"  v-model="payment_method" class="form-control select">
                                        ...
                                        <option v-bind:key="method.id" v-for="method in pay_methods" :value="method.id">{{ method.name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div v-if="addresses.length >= 1" class="form-group row">
                                <div class="col-12">
                                    <select id="addresses" name="addresses"  v-model="address" class="form-control select">
                                        ...
                                        <option v-bind:key="address.id" v-for="address in addresses" :value="address.id">{{ address.ship_to_name }} {{address.address_1}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        class="btn btn-primary" v-on:click.prevent.stop="updateAddress()">Save</button>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    v-on:click="startCheckout()" :disabled="notReady">
                                        Checkout
                                    </button>
                                </div>
                            </div>
